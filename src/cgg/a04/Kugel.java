...
public record Kugel(Point point, double radius, Color color) implements Shape{
  ...
  public Hit intersect(Ray ray) {
    ...
    if (0 <= discriminant) {
        ...
        if (ray.isValid(t1) && t1 < t2) {
            ...
            return new Hit(t1, ray.pointAt(t1), Vector.normalize(n), color);
        }

        if (ray.isValid(t2) && t2 < t1) {
            Direction n = Vector.divide(Vector.subtract(ray.pointAt(t2), point), radius);
            return new Hit(t2, ray.pointAt(t2), Vector.normalize(n), color);
        }
    }
    return null;
}
